...
const MyChildPage = () => {
   ...
   const recentActivity = [
      { type: "assignment", title: "Completed Math Quiz", date: "Today", status: "completed", score: "85%" },
      { type: "course", title: "Started Science Chapter 4", date: "Yesterday", status: "in-progress" },
      { type: "assignment", title: "English Essay Submission", date: "2 days ago", status: "pending-review" }
   ];

   const upcomingAssignments = [
      { title: "History Project", dueDate: "Feb 28, 2025", status: "not-started" },
      { title: "Science Lab Report", dueDate: "Mar 3, 2025", status: "in-progress" },
   ...
   const StatusIndicator = ({ status }) => {
      ...
      return <span className={`inline-block w-3 h-3 rounded-full ${statusStyles[status] || "bg-gray-400"} mr-2`}></span>;
   };
                     ...
                     <div className="flex justify-between items-start">
                        <div className="flex items-center gap-4">
                           <div className="bg-main/10 h-24 w-24 rounded-full flex items-center justify-center">
                              <User size={40} className="text-main" />
                           </div>
                           <div>
                              <h2 className="text-2xl font-semibold">{selectedChildData.name}</h2>
                              <p className="text-gray-600">{selectedChildData.grade}</p>
                              <div className="mt-2 flex gap-4">
                                 <div className="flex items-center gap-1 text-sm">
                                    <Award size={16} className="text-main" />
                                    <span>{selectedChildData.averageGrade}</span>
                                 </div>
                                 <div className="flex items-center gap-1 text-sm">
                                    <Calendar size={16} className="text-main" />
                                    <span>{selectedChildData.attendance} Attendance</span>
                                 </div>
                                 <div className="flex items-center gap-1 text-sm">
                                    <Clock size={16} className="text-main" />
                                    <span>{selectedChildData.upcomingAssignments} Due soon</span>
                                 </div>
                              </div>
                           </div>
                        </div>
                           ...
                           <div key={i} className="space-y-2">
                              <div className="flex justify-between items-center">
                                 <div className="flex items-center gap-2">
                                    <BookMarked size={18} className="text-main" />
                     ...
                     <div className="space-y-4">
                        <div className="bg-gray-50 p-4 rounded-lg">
                           ...
                           <div className="grid grid-cols-7 gap-1 h-16">
                              {[25, 45, 15, 60, 80, 30, 20].map((height, i) => (
                        ...
                        <div className="grid grid-cols-2 gap-3">
                           <div className="text-center p-3 bg-gray-50 rounded-lg">
                              <BarChart size={22} className="text-main mx-auto mb-1" />
                              <p className="text-2xl font-bold text-main">86%</p>
                              <p className="text-xs text-gray-600">Avg. Score</p>
                           </div>
                           <div className="text-center p-3 bg-gray-50 rounded-lg">
                              <Award size={22} className="text-main mx-auto mb-1" />
                              <p className="text-2xl font-bold text-main">4</p>
