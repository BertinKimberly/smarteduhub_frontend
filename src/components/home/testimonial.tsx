    ...
    <section id="testimonials" className="bg-white py-24 sm:py-32">
      <div className="container mx-auto px-4">
        ...
        <h3 className="md:w-3/4 lg:w-2/3 mx-auto text-xl text-center text-gray-600 mb-12">
          Join over 1,000 schools and 50,000+ students who are experiencing the transformative power of personalized education.
        </h3>
                  ...
                  <CardContent className="pt-6 pb-0">
                    <div className="flex gap-1 pb-6">
                      {[...Array(5)].map((_, i) => (
                        <Star 
                          key={i}
                          className={`size-4 ${i < Math.floor(review.rating) ? "fill-blue-600 text-blue-600" : "fill-gray-300 text-gray-300"}`} 
                        />
                      ))}
                    </div>
                    ...
                    <div className="flex flex-row items-center gap-4">
                      <Avatar>
                        <AvatarImage
                          ...
                          alt={review.name}
                        />
                        <AvatarFallback>{review.name.charAt(0)}{review.userName.charAt(0)}</AvatarFallback>
                      </Avatar>

                      <div className="flex flex-col">
                        <CardTitle className="text-lg text-gray-900">{review.name}</CardTitle>
                        <CardDescription className="text-gray-600">{review.userName}</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                </Card>
